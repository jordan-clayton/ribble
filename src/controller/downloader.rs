...
// NOTE: when making a downlaod request in the controller, spawn a wrapper thread that spawns the
...
impl DownloadRequest {
    pub(crate) fn new() -> Self {
        Self { url: Default::default(), file_name: Default::default(), directory: Default::default(), return_sender: None }
    }
...
impl DownloadEngineState {
    fn new(incoming_jobs: Receiver<DownloadRequest>, worker_sender: Sender<RibbleWorkerHandle>, progress_sender: Sender<ProgressMessage>) -> Self {
        Self { incoming_jobs, worker_sender, progress_sender }
    }
    ...
    fn start_download(&self, job: DownloadRequest) -> RibbleWorkerHandle {
        std::thread::spawn(move || {
            ...
            let progress_job = Progress::new_determinate("Downloading model.", sync_downloader.total_size() as u64);
            let (id_sender, id_receiver) = get_channel(1);
            let progress_message = ProgressMessage::Request { job: progress_job, source: id_sender };
            if self.progress_sender.send(progress_message).is_err() {
                // TODO: LOGGING -> this should only happen if the app should be closing.
            }

            let progress_id = match id_receiver.recv() {
                Ok(id) => Some(id),
                Err(_) => {
                    // TODO: logging -> same thing, only fails on sender being deallocated
                    // indicating progress engine is down.
                    None
                }
            };
            ...
            let progress_closure = move |n: usize| {
                ...
                let update = ProgressMessage::Increment { job_id: progress_id.unwrap(), delta: n as u64 };
            ...
            let console_message = ConsoleMessage::Status(format!("Successfully downloaded {}", &file_name));
...
impl DownloadEngine {
    ...
    pub(super) fn new(incoming_jobs: Receiver<DownloadRequest>, worker_sender: Sender<RibbleWorkerHandle>, progress_sender: Sender<ProgressMessage>) -> Self {
        let inner = Arc::new(DownloadEngineState::new(incoming_jobs, worker_sender, progress_sender));
        let thread_inner = Arc::clone(&inner);

        let worker = std::thread::spawn(move || {
            while let Ok(download_job) = thread_inner.incoming_jobs.recv() {
                if thread_inner.worker_sender.send(thread_inner.start_download(download_job)).is_err() {
