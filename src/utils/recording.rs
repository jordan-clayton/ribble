...
use realfft::num_traits::{FromPrimitive, NumCast, ToPrimitive, Zero};
...
pub fn cast_to_f64<T: NumCast + FromPrimitive + ToPrimitive + Zero>(source: &[T], dest: &mut [f64]) {
    let cast = source.iter().map(|n| n.to_f64().expect("Failed to cast T to f64")).collect::<Vec<f64>>();
    let cast = cast.as_slice();
    dest.copy_from_slice(cast);
}

pub fn cast_to_f32<T: NumCast + FromPrimitive + ToPrimitive + Zero>(source: &[T], dest: &mut [f32]) {
    let cast = source.iter().map(|n| n.to_f32().expect("Failed to cast T to f32")).collect::<Vec<_>>();
    let cast = cast.as_slice();
    dest.copy_from_slice(cast);
}
...
pub fn bandpass_filter(samples: &mut [f32], sample_rate: f32, f_central: f32) {
    ...
    let f0 = f_central.hz();
    let coeffs = Coefficients::<f32>::from_params(Type::BandPass, fs, f0, Q_BUTTERWORTH_F32).expect("Cutoff does not adhere to Nyquist Freq");
...
pub fn fft_analysis(samples: &[f32], result: &mut [f32; constants::NUM_BUCKETS]) {
    ...
    // Build frames
    let frames = welch_frames(&window_samples, Some(constants::NUM_BUCKETS), None).expect("Failed to build frames");
    ...
    let mut max_mag = 1.0;
    let log_average_signal_density: Vec<f32> = frames.iter().map(|frame| {
        process_fft(frame, fft.clone(), &mut input, &mut output);
        let mag = (output.iter().map(|c| c.norm_sqr()).sum::<f32>() / (frame.len() as f32).powi(2)).ln();
        if mag > max_mag {
            max_mag = mag;
        }
        mag
    ...
    let normalized_signal: Vec<f32> = log_average_signal_density.iter().map(|n| {
        *n / max_mag
    }).collect();

    assert_eq!(normalized_signal.len(), constants::NUM_BUCKETS, "Grouping failed. Expected: {}, Actual: {}", constants::NUM_BUCKETS, normalized_signal.len());
...
#[inline]
fn process_fft(samples: &[f32], fft: Arc<dyn RealToComplex<f32>>, input: &mut [f32], spectrum: &mut [Complex32]) {
    input.copy_from_slice(samples);

    fft.process(input, spectrum).expect("Slice with incorrect length supplied to fft");
}

// TODO: implement proper errors.
fn welch_frames(windowed_samples: &[f32], num_segments: Option<usize>, overlap: Option<f32>) -> Result<Vec<Vec<f32>>, WhisperRealtimeError> {
    let mut len = windowed_samples.len();
    let k = num_segments.unwrap_or(4);
    let a = overlap.unwrap_or(0.5);

    if len < 4 {
        return Err(WhisperRealtimeError::new(WhisperRealtimeErrorType::ParameterError, format!("Invalid sample size {}, minimum is 4", len)));
    }
    ...
    let frames: Vec<Vec<f32>> = samples.windows(l).step_by(overlap).map(|s| s.to_vec()).collect();

    assert!(frames.len() > 1, "Failed to group into frames");
    assert!(frames[0].len().is_power_of_two(), "Failed to group in powers of 2");
    Ok(frames)
}
...
fn smoothing(current: &mut [f32], target: &[f32], dt: f32) {
    assert_eq!(current.len(), target.len(), "Incorrect sizes. Current: {}, Target: {}", current.len(), target.len());
